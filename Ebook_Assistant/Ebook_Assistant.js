...
(function () {
	...
	*/
	...
	(function () {
	  ...
	  function queryLibrary(isbn) {
	    ...
	    GM_xmlhttpRequest({
	      ...
	      onload: function (responseDetail) {
	        ...
	        if ((response === null || response === void 0 ? void 0 : response.numFound) > 0) {
	  ...
	  try {
	    var _regexSubtitle$exec;

	    var regexSubtitle = /<span class="pl">\s*副标题:?<\/span>\s*:?\s*([\s\S]+?)<br\/?>/gi;
	    subtitle = (_regexSubtitle$exec = regexSubtitle.exec(document.documentElement.innerHTML.replace(/&nbsp;/gi, " "))) === null || _regexSubtitle$exec === void 0 ? void 0 : _regexSubtitle$exec[1].trim();
	  } catch (e) {
	    console.log(e);
	  }
