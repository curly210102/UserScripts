(function () {
      ...
      onload: function (responseDetail) {
        ...
        if (result.errmsg == "") {
          ...
          if ($('.online-type[data-ebassistant="read"]').length) {
            partnerTemplate =
              '<div class="online-read-or-audio"> <a class="impression_track_mod_buyinfo" target="_blank" href="{templateUrl}"> <img src="https://ebooks-assistant.oss-cn-guangzhou.aliyuncs.com/icon_weread.png" width="16" height="16"> <span>微信读书</span> </a> </div>';
            $('.online-type[data-ebassistant="read"]').append(
              partnerTemplate.replace("{templateUrl}", duokanUrl)
            );
          } else if ($('.online-type[data-ebassistant="audio"]').length) {
            partnerTemplate =
              '<div class="online-type" data-ebassistant="read"> <span>在线试读：</span> <div class="online-read-or-audio"> <a class="impression_track_mod_buyinfo" target="_blank" href="{templateUrl}" one-link-mark="yes"> <img src="https://ebooks-assistant.oss-cn-guangzhou.aliyuncs.com/icon_weread.png" width="16" height="16"> <span>微信读书</span> </a> </div></div>';
            $('.online-type[data-ebassistant="audio"]').before(
              partnerTemplate.replace("{templateUrl}", duokanUrl)
            );
          } else {
            partnerTemplate =
          ...
          var buyItemTemplate =
            '<li> <div class="cell price-btn-wrapper"> <div class="vendor-name"> <a target="_blank" href="{templateUrl}"> <span><img src="https://ebooks-assistant.oss-cn-guangzhou.aliyuncs.com/icon_weread.png" style="border-radius: 50%; box-shadow: 0 0 1px 0 rgba(0,0,0,0.6);" width="16" height="16" border="0">&nbsp;微信读书</span> </a> </div> <div class="cell impression_track_mod_buyinfo"> <div class="cell price-wrapper"> <a target="_blank" href="{templateUrl}"> <span class="buylink-price "> {templatePrice}元 </span> </a> </div> <div class="cell"> <a target="_blank" href="{templateUrl}" class="buy-book-btn e-book-btn"> <span>购买电子书</span> </a> </div> </div> </div> </li>';
          buyItemTemplate = buyItemTemplate.replaceAll(
            "{templateUrl}",
            duokanUrl
          );
          buyItemTemplate = buyItemTemplate.replace(
            "{templatePrice}",
            duokanPrice
          );
  ...
  function queryLibrary(isbn) {
    ...
    GM_xmlhttpRequest({
      method: "GET",
      url:
        "https://my1.hzlib.net/opac/api/search?q=" +
        isbn +
        "&searchType=standard&isFacet=true&view=standard&searchWay=isbn&rows=10&sortWay=score&sortOrder=desc&searchWay0=marc&logical0=AND&wt=json",
      headers: {
        "User-agent": window.navigator.userAgent,
      },
      onload: function (responseDetail) {
        ...
        if (response?.numFound > 0) {
          ...
          docs.forEach(function (doc) {
            ...
            GM_xmlhttpRequest({
              method: "GET",
              url:
                "https://my1.hzlib.net/opac/book/holdingPreviews?bookrecnos=" +
                bookid +
                "&return_fmt=json",
              headers: {
                "User-agent": window.navigator.userAgent,
              ...
              onload: function (responseDetail) {
                ...
                const books = result.previews[bookid].filter((item) =>
                  libcodes.has(item.curlib)
                );
                console.log(result);
  ...
  try {
    $(".online-partner .online-type:nth-child(1)").attr(
      "data-ebassistant",
      "read"
    );
    $(".online-partner .online-type:nth-child(2)").attr(
      "data-ebassistant",
      "audio"
    );
  } catch (e) {
  ...
  var linkedData = regexLinkedData
  ...
  try {
    var regexSubtitle =
      /<span class="pl">\s*副标题:?<\/span>\s*:?\s*([\s\S]+?)<br\/?>/gi;
    subtitle = regexSubtitle
      .exec(document.documentElement.innerHTML.replace(/&nbsp;/gi, " "))?.[1]
      .trim();
  } catch (e) {
    console.log(e);
  }
  ...
  try {
    var regexTranslator =
      /<span class="pl">\s*译者:?<\/span>\s*:?\s*<a[^>]+>([\s\S]+?)<\/a>/gi;
  ...
  var regexPublisher =
    /<span class="pl">\s*出版社:?<\/span>\s*:?\s*([\s\S]+?)<br\/?>/gi;
  var publisher = regexPublisher
    .exec(document.documentElement.innerHTML.replace(/&nbsp;/gi, " "))[1]
    .trim();
